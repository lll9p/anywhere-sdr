...
pub fn read_rinex_nav_all(
    ...
    iono_utc: &mut IonoUtc, fname: &dyn AsRef<Path>,
) -> anyhow::Result<usize> {
    ...
    for (iline, line) in lines.skip(lines_of_headers).enumerate() {
        ...
        if iline % 8 == 7 {
            ...
            data[ieph][sv].n = (GM_EARTH
                / (data[ieph][sv].A * data[ieph][sv].A * data[ieph][sv].A))
                .sqrt()
                + data[ieph][sv].deltan;
            data[ieph][sv].sq1e2 =
                (1.0 - data[ieph][sv].ecc * data[ieph][sv].ecc).sqrt();
